...
- 그러나 여기서 다루는 주제는 정확하게는 리액트 18부터 도입된 `동시성 렌더링`에 관한 이야기이다.
  - 이 `동시성 렌더링`도 사실 16부터 존재했었지만 실험적 기능이었고 정식화가 18부터 된 것
  - 18에서도 Root API 중 어떤것을 사용하는가에 달렸다.
    - `ReactDOM.createRoot`: `Concurrent renderer`사용 (동시성 렌더링)
    - `ReactDOM.render`: `Legacy renderer`사용 (동기 렌더링)
