...
- ```reconciler 가 react element를 fiber로 확장한다 즉, reconciler 가 hook을 알고 있다``` <br> 

#### reconciler 가 hook을 어떻게 react 코어로 전달하는가

1. react 코어 패키지는 hook 을 사용하기 위해 외부 (reconciler 패키지)에서 주입받음

- react
  - React <- ReactHooks <- ReactCurrentDispatcher <- ReactSharedInternals
    <- react/ReactSharedInternals.js 는 객체에 property로 외부 모듈을 할당 받는다.
